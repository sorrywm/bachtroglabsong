function RunClusteringVirilisFSCSSpecNoise_MATT(wav_file, parameterFile)
...
%
    ...
    pars = loadParameters(parameterFile)
    outdir = pars.outdir;
    prevtempfile = pars.prevtempfile;
    whichsignal = pars.whichsignal;
    minpulse = pars.minpulse;
    maxIPI = pars.maxIPI;
    ipiptl = pars.ipiptl;
    doplots = pars.doplots;
    remnoise = pars.remnoise;
    numpulsemax = pars.numpulsemax;
    diffThreshold = pars.diffThreshold;
    pulsetrainmax = pars.pulsetrainmax;
    sigmaThreshold = pars.sigmaThreshold;
    filtcut = pars.filtcut;
    fs = pars.fs;
    femalecut = pars.femalecut;
    baseline_quantile = pars.baseline_quantile;
    template_pca_dimension = pars.template_pca_dimension;
    domalefemale = pars.domalefemale;
    plotanalyzerdir = pars.plotanalyzerdir;
    segmenterdir = pars.segmenterdir;
    chronuxdir = pars.chronuxdir;
    butterdir = pars.butterdir;
    % These parameters appear to be deprecated.
    %segopts = pars.segopts;
    %isshort = pars.isshort;
    %analyzerdir = pars.analyzerdir;
    wmoptions = struct('fs',fs,'template_pca_dimension',...
                           template_pca_dimension, 'sigmaThreshold',...
                           sigmaThreshold,'baseline_quantile',...
                           baseline_quantile); %changed baseline_quantile and sigmaThreshold
    ...
    matoutname = strcat(outdir,wavbase,ext,'_data');
    ...
    if exist('song_range','var')==1
        if length(song_range) > 0
            ...
            matoutname = strcat(outdir,wavbase,ext,num2str(song_range(1)),'-',num2str(song_range(2)),'_data');
...
%    maxIPI = 0.08; %maximum IPI before a new bout is formed (in s)
...
function pars = loadParameters(parameterFileRelative)
    % Initialize variables that will be used as parameters for the
    %   analysis, and assign values specified in parameterFile.
    % Returns a ParameterClass object (pars), which is a collection of
    %   these variables.

    % First, find absolute path to the parameter file.
    [~, attributes] = fileattrib(parameterFileRelative);
    parameterFile = attributes.Name;
    % Create a copy of parameterFile without its comments because
    %   readtable() cannot handle commented lines.
    % A microsecond-level timestamp is used to create unique filenames,
    %   which prevents parallel batch executions deleting the _nocomments file.
    [pFilePath, pFileName, ~] = fileparts(parameterFile);
    parameterFileNoComments = strcat(pFilePath, '/', pFileName, ...
                                     strrep(num2str(clock),' ',''), ...
                                     '_nocomments.txt');
    pFileID = fopen(parameterFile, 'r');
    pFileID_nocomments = fopen(parameterFileNoComments, 'at');
    fileLine = fgets(pFileID);
    while ischar(fileLine)
        if regexp(fileLine, '^[^%]') % First, is the line explicitly commented?
            if regexp(fileLine, '=') % Next, does the line define a parameter?
                fprintf(pFileID_nocomments, '%s', fileLine); % Copy parameters.
            end
        end
        fileLine = fgets(pFileID);
    end
    fclose(pFileID); fclose(pFileID_nocomments);
    % Load the no-comments parameters file into a Matlab table.
    T = readtable(parameterFileNoComments, 'Delimiter', '=', 'ReadVariableNames', true);
    % Convert that table to a Matlab structure.
    TS = table2struct(T);
    delete(parameterFileNoComments); % Delete the temporary parameter file.
    % Loop through the Matlab structure to assign parameter values to variables.
    pars = ParametersClass;
    % Assume the parameters file is in the output folder.
    pars.outdir = strcat(pFilePath, '/');
    for i = 1:length(TS)
        switch TS(i).Parameter
            case 'outdir' % Remove this case once the default assumption works.
                %[status, attributes] = fileattrib(TS(i).Value);
                %pars.outdir = strcat(attributes.Name, '/');
            case 'prevtempfile'
                [~, attributes] = fileattrib(TS(i).Value);
                pars.prevtempfile = attributes.Name;
            case 'whichsignal'
                % Convert a plain text list of template numbers
                %   (e.g. '[1 2 3]') to a Matlab list.
                whichsignal_raw = TS(i).Value;
                % Remove the [ ] and convert the string into a list.
                whichsignal_list = strsplit(strrep(strrep(whichsignal_raw, '[', ''), ']', ''), ' ');
                % Convert the list of strings to a list of numbers.
                for i = 1:length(whichsignal_list)
                    whichsignal(i) = str2num(char(whichsignal_list(i)));
                end
                pars.whichsignal = whichsignal;
            case 'minpulse'; pars.minpulse = str2num(TS(i).Value);
            case 'maxIPI'; pars.maxIPI = str2num(TS(i).Value);
            case 'ipiptl'; pars.ipiptl = TS(i).Value;
            case 'doplots'; pars.doplots = TS(i).Value;
            case 'remnoise'; pars.remnoise = TS(i).Value;
            case 'numpulsemax'; pars.numpulsemax = str2num(TS(i).Value);
            case 'diffThreshold'; pars.diffThreshold = str2num(TS(i).Value);
            case 'pulsetrainmax'; pars.pulsetrainmax = str2num(TS(i).Value);
            case 'sigmaThreshold'; pars.sigmaThreshold = str2num(TS(i).Value);
            case 'filtcut'; pars.filtcut = str2num(TS(i).Value);
            case 'fs'; pars.fs = str2num(TS(i).Value);
            case 'femalecut'; pars.femalecut = str2num(TS(i).Value);
            case 'baseline_quantile'; pars.baseline_quantile = str2num(TS(i).Value);
            case 'template_pca_dimension'; pars.template_pca_dimension = str2num(TS(i).Value);
            case 'domalefemale'; pars.domalefemale = TS(i).Value;
            case 'plotanalyzerdir'
                [~, attributes] = fileattrib(TS(i).Value);
                pars.plotanalyzerdir = strcat(attributes.Name, '/');
            case 'segmenterdir'
                [~, attributes] = fileattrib(TS(i).Value);
                pars.segmenterdir = strcat(attributes.Name, '/');
            case 'chronuxdir'
                [~, attributes] = fileattrib(TS(i).Value);
                pars.chronuxdir = strcat(attributes.Name, '/');
            case 'butterdir'
                [~, attributes] = fileattrib(TS(i).Value);
                pars.butterdir = strcat(attributes.Name, '/');
            % These parameters appear to be deprecated.
            %case 'segopts'; pars.segopts = TS(i).Value;
            %case 'isshort'; pars.isshort = TS(i).Value;
            %case 'analyzerdir'; pars.analyzerdir = TS(i).Value;
        end
    end
end
