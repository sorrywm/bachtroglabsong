function RunClusteringOneChannelFilt(wav_file, prevtempfile, whichsignal, isshort, fs, ipiptl)
%Modify the following to the location of your versions of these programs.
segmenterdir = 'D:/BackupFromDesktop072815/MATLABCode/MATLAB/Flysong/FlySongSegmenter/';
plotanalyzerdir = 'D:/fly_song_analyzer_032412/fly_song_analyzer_032412/';
butterdir='D:/BackupFromDesktop072815/MATLABCode'; %Where is tybutter.m saved?
...
if strcmp(ipiptl, 'IPI') == 1
    ...
    for i=1:CC.NumObjects
        ...
        if numPulses(i) >= 5
            tokeep = [tokeep ts(CC.PixelIdxList{i}) ts(CC.PixelIdxList{i}(end)+1)];
        end
    end
    tokeep = sort(tokeep);
    w5all = diff(tokeep);
