function RunClusteringMinimal(wav_file, prevtempfile, ...
    ipiptl, segopts, outdir, doplots, remnoise)
    %Modified 5/18/16: no longer takes whichsignal (specified in
    %outputData.isNoise)
    %Modified 4/15/16: use FlySongClusterSegment version of assignDataToTemplates
    %To do: Save pulse train parameters togetherlo so cutoff can be adjusted
    ...
    clearvars -except wav_file prevtempfile ipiptl segopts outdir doplots remnoise diffThreshold;
    ...
    if exist(segopts,'file') == 2
        ...
        if exist('sigmaThreshold','var')
            relvars = {'fs','template_pca_dimension','sigmaThreshold'};
            wmoptions = load(segopts,relvars{:});
        else
            relvars = {'fs','template_pca_dimension'};
            wmoptions = load(segopts,relvars{:});
            wmoptions.sigmaThreshold = 1.1;
         end
    else
        ...
        diffThreshold = 25;
        sigmaThreshold = 1.1;
        baseline_quantile = 0.9; %equal to default
        ...
        save(segopts,'analyzerdir','plotanalyzerdir','segmenterdir','chronuxdir', ...
             ...
             'filtcut','isshort','domalefemale','minpulse','baseline_quantile',...
             'diffThreshold');
    end
    ...
    fprintf('Running clustering for %s.\n',wav_file);

    %Load in previously computed templates.
    fprintf('Loading previous templates: %s.\n', prevtempfile);
    load(prevtempfile); %now contains outputData
    whichsignal=find(~outputData.isNoise);
    ...
    %The following are for the older version of FlySongClusterSegment.
    ...
    %isnoise = true[1:length(newtemplates)];
    ...
    if exist('dataoutname','file') ~= 2
        ...
        try
            ...
            %[groupings,peakIdxGroup,likes,allPeakIdx,allNormalizedPeaks,...
            %coeffs,projStds,baselines] = assignDataToTemplates(dfilt,newtemplates,...
            %isnoise,wmoptions);
            %New version of FlySongClusterSegment:
            [groupings,peakIdxGroup,likes,allPeakIdx,allNormalizedPeaks] = ...
        ...
        %Save these variables... including which templates were called signals and options.
        save(dataoutname,'groupings','peakIdxGroup','likes','allPeakIdx', ...
            'allNormalizedPeaks','prevtempfile','wmoptions');
    else
        load(dataoutname);
    end
    ...
    if strcmp(doplots,'TRUE')
        %no longer retains any peaks from noise templates
    ...
    w = wall(wall <= maxIPI);
    fprintf('Number of IPI measurements less than %d s %d\n', maxIPI, length(w));
