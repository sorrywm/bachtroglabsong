function CreateSpeciesTemplatesFiltNewFSCS(initdir, wavbase, segopts, ...
    diffThreshold, sigmaThreshold)
    %Saving options file currently relies on the following toolbox:
    ...
    plotanalyzerdir = '/Users/wynnmeyer/repos/FlySongClusterSegment/';
    ...
    savefig(strcat(outdir,'/',outname,newtempoutname,'dT',...
        num2str(diffThreshold),'sT',num2str(sigmaThreshold),...
        'TemplateHistograms.fig'));
    close;
    sprintf('All peaks found (new code): %d\n',length(allPeakIdx))
    %save(strcat(outdir,'/',outname,newtempoutname,'dT',diffThreshold,'.mat'),'newtemplates');
    save(strcat(outdir,'/',outname,newtempoutname,'dT',num2str(diffThreshold),...
        'sT',num2str(sigmaThreshold),'.mat'),'outputData');
    save(strcat(outdir,'/',outname,'dT',num2str(diffThreshold),...
        'sT',num2str(sigmaThreshold),'outputfromcreateTemplates.mat'),'allPeakIdx',...
    ...
    save(strcat(outdir,'/',outname,'dT',num2str(diffThreshold),...
        'sT',num2str(sigmaThreshold),'assigned.mat'),'groupings','peakIdxGroup',...
    ...
    savefig(strcat(outdir,'/',outname,newtempoutname,'dT',num2str(diffThreshold),...
        'sT',num2str(sigmaThreshold),'AllColored.fig'));
end