function addCourtshipInfoAndPlot(filename, testVar)  %string of column name
    %This function is a wrapper that calls the subfunctions.
    %if testVar is 'humidity' or 'temp' or '...'
    %testVar is case sensitive
    [pathstr,name,ext] = fileparts(filename);
    plotname = [pathstr '/' name testVar]; %base for plotting two jpeg files
    [var, note] = createArrays(filename, testVar);  %var must always be int array
    court = findCourtship(var, note);
    createPlot(var, court, testVar, plotname);
end

function [varArray, noteArray] = createArrays(file, varName) 
    %also save in table format so courtship can be easily added
...
function createPlot(varArray, courtshipArray, varName, plotName)
    ...
    title(strcat('Graphic Representation of Number that Successfully Courted at a Specific', varName))
    xlabel(varName)
    ylabel('Number of Flies (in pairs)')
    legend('Total Flies', 'Flies that Courted')
    print([plotName 'Number.jpeg'], '-djpeg')
    ...
    print([plotName 'Percent.jpeg'], '-djpeg')
end    
